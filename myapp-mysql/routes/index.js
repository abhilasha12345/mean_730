...
/*Middleware to manage cookies in application */
var cunm=""
var cpass=""
router.use('/login', (req, res, next)=>{
  if(req.cookies.cunm!=undefined)
  {
    ...
    cpass=cryptoapi.myDecode(req.cookies.cpass)		
  }  
  next()
});
...
router.get('/about',(req, res, next)=>{
  /*res.cookie('cunm','admin',{'maxAge':60000*60*24})
  res.cookie('cpass','admin123',{'maxAge':60000*60*24})
...
router.post('/login', function(req, res, next) {
  indexModel.login(req.body).then((result)=>{
  	if(result.length>0)
  	{
  		...
  		{
  		res.cookie('cunm',req.body.email,{'maxAge':60000*60*24*365})
  		res.cookie('cpass',cryptoapi.myEncode(req.body.password),{'maxAge':60000*60*24*365})
  		}
  		...
  		res.render('login',{'output':'Invalid User or Verify your account from inbox......','cunm':cunm,'cpass':cpass});
  }).catch((err)=>{
  	console.log(err)
  })
});
